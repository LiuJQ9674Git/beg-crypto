...
public class SM2CertUtilTest {
    ...
    public void testGetBCECPublicKey() {
        try {
            ...
            BCX509CertFace.makeCertificate("target/test4.sm2.cer",
            ...
            byte[] priKey= BCX509CertFace.readPrivateKey("target/test4.sm2.pri");
    ...
    public void testVerifyCertificateFromFile() {
        try {
            ...
            //根证书
            X509Certificate rootCACert= BCX509CertFace.readPublicKey(ROOT_CERT_PATH);
            X509Certificate midCACert = BCX509CertFace.readPublicKey(MID_CERT_PATH);
            X509Certificate userCert = BCX509CertFace.readPublicKey(USER_CERT_PATH);

            byte[] rootKey= BCX509CertFace.readPrivateKey(ROOT_PRI_PATH);
            byte[] midKey= BCX509CertFace.readPrivateKey(MID_PRI_PATH);
            byte[] userKey= BCX509CertFace.readPrivateKey(USER_PRI_PATH);
    ...
    public void testVerifyCertificate() {
        try {
            ...
            //根证书
            X500Name rootDN = BCX509CertFace.buildRootCADN();
            ...
            //保存私钥
            BCX509CertFace.savePriKey(ROOT_PRI_PATH, (BCECPrivateKey) rootKP.getPrivate(),
                (BCECPublicKey) rootKP.getPublic());
            ...
            //保存公钥
            BCX509CertFace.writePublicKey(ROOT_CERT_PATH, rootCACert.getEncoded());
            ...
            BCX509CertFace.savePriKey(MID_PRI_PATH, (BCECPrivateKey) midKP.getPrivate(),
                (BCECPublicKey) midKP.getPublic());

            //通过根证书创建
            X509Certificate midCACert = rootCertMaker.makeSubCACert(midCSR);

            BCX509CertFace.writePublicKey(MID_CERT_PATH, midCACert.getEncoded());

            BCX509CertMaker midCertMaker = new BCX509CertMaker(midKP, certExpire,
                    midDN, snAllocator);
            KeyPair userKP = SM2Util.generateKeyPair();
            X500Name userDN = BCX509CertFace.buildSubjectDN();
            ...
            BCX509CertFace.savePriKey(USER_PRI_PATH, (BCECPrivateKey) userKP.getPrivate(),
                (BCECPublicKey) userKP.getPublic());
            X509Certificate userCert = midCertMaker.makeSSLEndEntityCert(userCSR);

            BCX509CertFace.writePublicKey(USER_CERT_PATH, userCert.getEncoded());

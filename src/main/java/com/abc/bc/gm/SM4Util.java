...
public class SM4Util extends GMBaseUtil {
    ...
    public static String encrypt_String_ECB_NoPadding(String plainText, byte[] key) {
        try {
            byte[] ciphertext = encrypt_ECB_NoPadding(plainText.getBytes(), key);
    ...
    public  static String decrypt_String_ECB_NoPadding(String plainText, byte[] key) {
        byte[] plainByte = new byte[0];
        try {
            plainByte = decrypt_ECB_NoPadding(Base64.decodeBase64(plainText.getBytes()), key);
        } catch (Exception e) {
            throw new RuntimeException("MS4解密失败:\t"+e.getMessage());
        }
